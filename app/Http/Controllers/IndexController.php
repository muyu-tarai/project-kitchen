...
class IndexController extends Controller
{
    public function index()
    {
        ...
        foreach ($openStores as $key => $store) {
            $openStores[$key]->ext = File::extension(!empty($store->store_image));
            $openStoreImageFromDropbox = base64_encode(Storage::disk('dropbox')->get(!empty($store->store_image)));
            $openStores[$key]->store_image = $openStoreImageFromDropbox;
        }
        foreach ($closeStores as $key => $store) {
            $closeStores[$key]->ext = File::extension(!empty($store->store_image));
            $closeStoreImageFromDropbox = base64_encode(Storage::disk('dropbox')->get(!empty($store->store_image)));
